...
| NASA beta scale mismatch + LCO chemistry label | `beta_NASA=0.021545` (fit on D_k=k scale, NASA LCO cells) was applied in cell_to_field_bridge and sobol_factor_ranking with D_cumul on Palmgren-Miner D scale — 262× scale error. Additionally, caveats in cell_to_field_report.json and README.md labelled NASA cells as "NMC/Sanyo NMC" when they are LCO (Sanyo UR18650E). The correctly-scaled `beta_Miner=5.663` predicts 25% cycling fade on Deng fleet vs 2–9% observed total, confirming LCO-to-NCM transfer is invalid regardless of D scale. | `absolute_contribution` table in sobol_ranking_report.json retracted (values preserved in `absolute_contribution_RETRACTED` for audit); chemistry label corrected to LCO throughout; cycling-negligibility justification confirmed as grounded in M2's own NCM fit (gamma_c=3.630 on Deng data) and temporal model's calendar-only RMSE match on Tier 3 — not in NASA beta transfer. | pending |
